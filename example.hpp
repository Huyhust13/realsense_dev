...
private:
    ...
    void render_frameset(const rs2::frameset &frames, const rect &r)
    {
        ...
        std::sort(supported_frames.begin(), supported_frames.end(), [](rs2::frame first, rs2::frame second) { return first.get_profile().stream_type() < second.get_profile().stream_type(); });
